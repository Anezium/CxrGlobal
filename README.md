...
- 認可フロー: グローバル版 Hi Rokid の AuthorizationActivity を呼び出し → token 取得
- AIDL バインディング: `IMediaStreamService` への接続 / 切断
- 各 AIDL メソッドの 1:1 ラッパー
  - CustomView: open / update / close / setIcons / 状態取得
  - Audio stream: PCM 16 kHz / mono / 16-bit リアルタイムストリーム取得
  - Photo: `takePhoto`
  - CustomCMD: `sendCustomCmd`
  - グラス側アプリ操作: install / uninstall / start / stop / queryInstalled
  - AI Event: AI session up/down / glass app resume change
...
- `AuthorizationHelper.isHiRokidInstalled(context)` が `false`: グローバル版 Hi Rokid 未インストール、または `<queries>` の登録漏れ。
